...
def overlay_hand_on_background(hand_img, background_img, mask):
    ...
    alpha = mask_np / 255.0
    combined_img_np = (alpha * hand_img_np) + ((1 - alpha) * background_img_np)
    combined_img = Image.fromarray(combined_img_np.astype(np.uint8), 'RGBA')
...
background_folder = 'test/resized_images'
output_folder = 'test/newdata'

# Perform augmentation
augment_dataset(dataset_folder, background_folder, output_folder, num_augmentations=5)
